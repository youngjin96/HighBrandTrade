...
@Component({
  selector: 'app-cloth-women',
  templateUrl: './cloth-women.component.html',
  styleUrls: ['./cloth-women.component.css']
})
export class ClothWomenComponent implements OnInit {
  ...
  getItem(db_name : string){  // getItem() 의 인자로 들어가는 이름의 collection에 접근을 할 거라고 설정
    this.itemCollection = this.db.collection<any>(db_name, (ref : CollectionReference) => {
      return ref.where('product', 'array-contains', 'cloth_women').orderBy('brand_name');
    });
